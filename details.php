...
if (isset($_GET['reaction_list'])) {
	...
	if ($reactionUsers) {
		...
		foreach ($reactionUsers as $reactionUser) {
			...
			echo '<a class="plus-reaction-user-link" href="'.profile_href($reactionUser).'">'.get_user_color((int) $reactionUser['class'], htmlspecialchars((string) $reactionUser['name'], ENT_QUOTES, 'UTF-8'), $reactionUser).'</a>';
			echo '<span class="plus-reaction-user-badge '.($isDislike ? 'plus-reaction-user-badge-dislike' : 'plus-reaction-user-badge-like').'">'.htmlspecialchars($reactionLabel, ENT_QUOTES, 'UTF-8').'</span>';
			echo '</div>';
		}
		echo '</div>';
	} else {
