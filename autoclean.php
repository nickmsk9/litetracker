...
///////////////////////////////////////////////////////////////////
//Удаление просроченный активиционных кодов
///////////////////////////////////////////////////////////////////
$secs = 15*24*(60*60); //Количество секунд , через которое код считается просроченным (15 дней)
$date = $db->safesql(get_date_time(gmtime() - $secs));
$sql = $db->query("SELECT * FROM confirm  WHERE date < '".$date."'");
while($arr = $db->get_row($sql) ) {
	$db->query("DELETE FROM confirm WHERE id = ".$arr['id']);
	$db->query("DELETE FROM users WHERE id = ".$arr['id_user']);
}

///////////////////////////////////////////////////////////////////
//Удаление просроченный активиционных кодов "Забыли пароль?"
///////////////////////////////////////////////////////////////////
$secs = 15*24*(60*60); //Количество секунд , через которое код считается просроченным (15 дней)
$date = $db->safesql(get_date_time(gmtime() - $secs));
